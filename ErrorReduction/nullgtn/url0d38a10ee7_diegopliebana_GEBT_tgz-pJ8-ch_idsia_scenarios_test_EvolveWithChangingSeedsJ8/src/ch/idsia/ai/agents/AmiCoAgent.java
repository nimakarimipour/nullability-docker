...
public class AmiCoAgent implements Agent {
    ...
    public boolean[] getAction(Environment observation) {
        ...
        int[] marioState = new int[] { observation.getMarioStatus(), observation.getMarioMode(), observation.isMarioOnGround() ? 1 : 0, observation.isMarioAbleToJump() ? 1 : 0, observation.isMarioAbleToShoot() ? 1 : 0, observation.isMarioCarrying() ? 1 : 0, observation.getKillsTotal(), observation.getKillsByFire(), observation.getKillsByStomp(), observation.getKillsByStomp(), observation.getKillsByShell() };
        ...
        for (int i = 0; i < action.length; ++i) ret[i] = (action[i] != 0);
        return ret;
    }
