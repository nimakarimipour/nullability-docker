...
public class Mario extends Sprite {
    ...
    // exclude pause for rendering changes
    ...
    public Mario(LevelScene world) {
        ...
        //        Mario.instance = this;
        this.world = world;
        // SK: in fact, this is already redundant due to using Agent
        keys = Scene.keys;
        // SK: in fact, this is already redundant due to using Agent
    ...
    public void move() {
        ...
        if (x > world.level.xExit * 16 - 16 && x < world.level.xExit * 16 + 2 * 16 && y < world.level.yExit * 16) {
    ...
    private boolean isBlocking(float _x, float _y, float xa, float ya) {
        ...
        if (((Level.TILE_BEHAVIORS[block & 0xff]) & Level.BIT_PICKUPABLE) > 0) {
            ...
            for (int xx = 0; xx < 2; xx++) for (int yy = 0; yy < 2; yy++) world.addSprite(new Sparkle(x * 16 + xx * 8 + (int) (Math.random() * 8), y * 16 + yy * 8 + (int) (Math.random() * 8), 0, 0, 0, 2, 5));
        }
        if (blocking && ya < 0) {
