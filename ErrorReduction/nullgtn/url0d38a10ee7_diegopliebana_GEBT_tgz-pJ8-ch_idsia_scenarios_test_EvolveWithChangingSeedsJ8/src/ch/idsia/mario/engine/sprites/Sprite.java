...
public class Sprite {
    ...
    public void render(Graphics og, @javax.annotation.Nullable float alpha) {
        if (!visible)
            return;
        ...
        if (GlobalOptions.isShowGrid) {
            if (this.kind == KIND_MARIO) {
                ...
                for (k = -rows / 2 - 1; k <= rows / 2; k++) og.drawLine((int) x - width / 2, (int) (y + k * htOfRow), (int) (x + width / 2), (int) (y + k * htOfRow));
                //                og.setColor(Color.RED);
                // vertical lines
                // width / (rows);
                int wdOfRow = 16;
                for (k = -columns / 2 - 1; k < columns / 2 + 1; k++) og.drawLine((int) (x + k * wdOfRow + 8), (int) y - height / 2 - 8, (int) (x + k * wdOfRow + 8), (int) (y + height / 2 - 8));
            }
    ...
    public void bumpCheck(@javax.annotation.Nullable int xTile, @javax.annotation.Nullable int yTile) {
    }

    public boolean shellCollideCheck(@javax.annotation.Nullable Shell shell) {
        return false;
    }
    ...
    public boolean fireballCollideCheck(@javax.annotation.Nullable Fireball fireball) {
        return false;
    }
}
