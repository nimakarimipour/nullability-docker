...
public class LevelGenerator {
    ...
    //ATTENTION: not cloned.
    private int[] cmdArgs;

    //TODO: fix LevelLengthMinThreshold
    // minimal length of the level. not using now
    ...
    private Level createLevel(long seed, int difficulty, int type) {
        ...
        if (type != LevelGenerator.TYPE_OVERGROUND) {
            //if not overground then there are no hill straight
    ...
    private int buildZone(int x, int maxLength, int maxHeight, int floor, int floorHeight) {
        ...
        switch(type) {
            ...
            case //TODO:check adding of HS in method code
            ODDS_HILL_STRAIGHT:
            ...
            case ODDS_DEAD_ENDS:
                {
                    if (//if method was not called from buildDeadEnds
                    floor == DEFAULT_FLOOR && deadEndsCount < cmdArgs[20]) {
                        deadEndsCount++;
...
*/
    ...
    private int buildDeadEnds(int x0, int maxLength) {
        ...
        //floor of pre dead end zone
        int floor = height - 2 - random.nextInt(2);
        // total zone length
        int length = 0;
        int preDeadEndLength = 7 + random.nextInt(10);
        //rest height
        int rHeight = floor - 1;
        //buildZone( x0, x0+preDeadEndLength, floor ); //build pre dead end zone
        length += buildStraight(x0, preDeadEndLength, true, floor, INFINITY_FLOOR_HEIGHT);
        buildBlocks(x0, x0 + preDeadEndLength, floor, true, 0, 0, true);
        //correct direction
        //true - top, false = bottom
        random.nextInt();
        //(random.nextInt() % (this.levelDifficulty+1));
        int k = random.nextInt(5);
        boolean direction = random.nextInt(k + 1) != 1;
        //Y coordinate of the top line of the separator
        int separatorY = 3 + random.nextInt(rHeight - 7);
        ...
        for (int x = nx; x < nx + depth; x++) {
            for (int y = 0; y < height; y++) {
                if (x - nx >= depth - wallWidth) {
                    if (//wall on the top
                    direction == RIGHT_DIRECTION_BOTTOM) {
                        if (// + separatorHeight )
                        y <= separatorY) {
                            level.setBlock(x, y, (byte) (1 + 9 * 16));
                        }
        ...
        for (int x = xo; x < xo + length; x++) {
            ...
            for (int y = 0; y < height; y++) {
                if (y >= floor && y <= floor + floorHeight) {
                    ...
                    if (// + floorHeight)
                    x == xCannon && y >= cannonHeight && y <= floor) {
                        if (y == cannonHeight) {
    ...
    private void buildBlocks(int x0, int x1, int floor, boolean pHB, int pS, int pE, boolean onlyHB) {
        ...
        if (onlyHB)
            hb = onlyHB;
        while (//minimal distance between the bricks line and floor is 4
        floor - 4 > 0) {
            if (//minimal number of bricks in the line is positive value
            (x1 - 1 - e) - (x0 + 1 + s) > 0) {
                    ...
                    if (hb && cmdArgs[27] != 0) {
                        ...
                        if (x != x0 + 1 && x != x1 - 2 && random.nextInt(3) == 0) {
                            if (canBuildBlocks(x, floor - 4, false)) {
                                blocksCount++;
                                if ((random.nextInt(4) == 0)) {
                                    //a rock with animated question symbol with flower. when broken becomes a rock
                                    level.setBlock(x, floor - 4, (byte) (4 + 2 + 1 * 16));
                                } else {
                                    //a brick with animated question symbol. when broken becomes a rock
                            ...
                            if (canBuildBlocks(x, floor - 4, false)) {
                                blocksCount++;
                                if (random.nextInt(4) == 0) {
                                    //a brick with a flower. when broken becomes a rock
    ...
    private void buildCoins(int x0, int x1, int floor, int s, int e) {
        ...
        if ((x1 - 1 - e) - (x0 + 1 + s) > 1) {
            for (int x = x0 + 1 + s; x < x1 - 1 - e; x++) {
                ...
                if (//if cell (x, floor-2) is empty
                level.getBlock(x, floor - 2) == 0) {
                    coinsCount++;
                    //coin
    ...
    private void decorate(int x0, int x1, int floor, boolean recurs) {
        ...
        if (!recurs)
            buildBlocks(x0, x1, floor, hb, s, e, false);
        int length = x1 - x0 - 2;
        if (length > 2) {
