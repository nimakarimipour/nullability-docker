...
public class LevelScene extends Scene implements SpriteContext {
    ...
    //passed to reset method. ATTENTION: not cloned.
    private int[] args;
        ...
        switch(ZLevel) {
            case (0):
                switch(el) {
                    ...
                    case 17:
                    case // brick with a hidden flower
                    18:
                        // prevents cheating
                        return 16;
                    // question brick, contains coin
                    case 21:
                    case // question brick, contains flower/mushroom
                    22:
                        // question brick, contains something
                        return 21;
                }
                return el;
            case (1):
                switch(el) {
                    ...
                    (15):
                        return 0;
                    case // Coin, irrelevant for the current contest
                    (34):
                        return 34;
                    case (-128):
                    case (-127):
                    ...
                    case // canon
                    (46):
                        // angry flower pot or cannon
                        return 20;
                }
                System.err.println("ZLevelMapElementGeneralization: Unknown value el = " + el + " Possible Level tiles bug; " + "Please, inform sergey@idsia.ch or julian@togelius.com. Thanks!");
                return el;
            case (2):
                switch(el) {
    ...
    public byte[][] getEnemiesObservationZ(int ZLevel) {
        ...
        int MarioXInMap = (int) mario.x / 16;
        int MarioYInMap = (int) mario.y / 16;
        for (int w = 0; w < enemiesZ.length; w++) for (int h = 0; h < enemiesZ[0].length; h++) enemiesZ[w][h] = 0;
        //        enemiesZ[Environment.HalfObsWidth][Environment.HalfObsHeight] = mario.kind;
        for (Sprite sprite : sprites) {
            if (sprite.kind == mario.kind)
                continue;
            if (sprite.mapX >= 0 && sprite.mapX >= MarioXInMap - Environment.HalfObsWidth && sprite.mapX <= MarioXInMap + Environment.HalfObsWidth && sprite.mapY >= 0 && sprite.mapY >= MarioYInMap - Environment.HalfObsHeight && sprite.mapY <= MarioYInMap + Environment.HalfObsHeight) {
        ...
        if (level != null && mario != null) {
            ...
            if (LevelMap) {
                ...
                for (int x = 0; x < levelScene.length; ++x) {
                    String tmpData = "";
                    for (int y = 0; y < levelScene[0].length; ++y) tmpData += mapElToStr(levelScene[x][y]);
                    ret.add(tmpData);
                }
            }
            ...
            if (mergedObservationFlag) {
                ...
                for (int x = 0; x < levelScene.length; ++x) {
                    String tmpData = "";
                    for (int y = 0; y < levelScene[0].length; ++y) tmpData += mapElToStr(mergedObs[x][y]);
                    ret.add(tmpData);
                }
            }
        } else
    ...
    public void tick() {
        ...
        if (paused) {
            ...
            //            int xCannon = 0;
            for (int x = (int) xCam / 16 - 1; x <= (int) (xCam + this.width) / 16 + 1; x++) for (int y = (int) yCam / 16 - 1; y <= (int) (yCam + this.height) / 16 + 1; y++) {
                int dir = 0;
    ...
    public void bump(int x, int y, boolean canBreakBricks) {
        ...
        if ((Level.TILE_BEHAVIORS[block & 0xff] & Level.BIT_BREAKABLE) > 0) {
            ...
            if (canBreakBricks) {
                ...
                for (int xx = 0; xx < 2; xx++) for (int yy = 0; yy < 2; yy++) addSprite(new Particle(x * 16 + xx * 8 + 4, y * 16 + yy * 8 + 4, (xx * 2 - 1) * 4, (yy * 2 - 1) * 4 - 8));
            } else {
                level.setBlockData(x, y, (byte) 4);
            }
        }
