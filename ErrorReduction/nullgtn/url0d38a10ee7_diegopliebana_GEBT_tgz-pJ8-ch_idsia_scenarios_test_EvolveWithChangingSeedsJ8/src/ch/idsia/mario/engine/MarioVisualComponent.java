...
public class MarioVisualComponent extends JComponent {
    ...
    public void CreateMarioComponentFrame(MarioVisualComponent m) {
        if (marioComponentFrame == null) {
            ...
            "Mario AI benchmark " + GlobalOptions.getVersionUID());
    ...
    public void tick() {
        ...
        msg = "";
        if (mario.keys != null) {
            for (int i = 0; i < Environment.numberOfButtons; ++i) msg += (mario.keys[i]) ? Scene.keysStr[i] : "      ";
        } else
            msg = "NULL";
    ...
    public void render(Graphics g, float alpha) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            //levelScene.
            bgLayer[i].render(g, levelScene.tick, alpha);
        }
        g.translate(-xCam, -yCam);
        for (// levelScene.
        // levelScene.
        Sprite sprite : levelScene.sprites) {
            if (sprite.layer == 0)
        ...
        if (mario.cheatKeys[Mario.KEY_DUMP_CURRENT_WORLD])
            for (int w = 0; w < level.width; w++) for (int h = 0; h < level.height; h++) level.observation[w][h] = -1;
        for (Sprite sprite : levelScene.sprites) {
            if (sprite.layer == 1)
                sprite.render(g, alpha);
            if (mario.cheatKeys[Mario.KEY_DUMP_CURRENT_WORLD] && sprite.mapX >= 0 && sprite.mapX < level.observation.length && sprite.mapY >= 0 && sprite.mapY < level.observation[0].length)
        ...
        if (mario.winTime > 0) {
            ...
            renderBlackout(g, mario.xDeathPos - xCam, mario.yDeathPos - yCam, (int) (GlobalOptions.VISUAL_COMPONENT_WIDTH - t));
        }
    ...
    private void drawProgress(Graphics g) {
        ...
        String progress_str = "";
        for (int i = 0; i < progressInChars - 1; ++i) progress_str += ".";
        progress_str += "M";
        try {
    ...
    public static void drawString(Graphics g, String text, int x, int y, int c) {
        ...
        for (int i = 0; i < ch.length; i++) g.drawImage(Art.font[ch[i] - 32][c], x + i * 8, y, null);
    }
    ...
    private void drawGrid(Graphics g, int length) {
        ...
        int k;
        for (k = 0; k < rows; k++) g.drawLine((int) mario.x, k * htOfRow, (int) (mario.x + width), k * htOfRow);
        int wdOfRow = width / (columns);
        for (k = 0; k < columns; k++) g.drawLine(k * wdOfRow, 0, k * wdOfRow, height);
    }
