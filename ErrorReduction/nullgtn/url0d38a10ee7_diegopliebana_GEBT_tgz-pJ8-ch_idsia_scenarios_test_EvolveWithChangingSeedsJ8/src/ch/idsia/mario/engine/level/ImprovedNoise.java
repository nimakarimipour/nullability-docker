...
public final class ImprovedNoise {
    ...
    public double noise(double x, double y, double z) {
        ...
        A = p[X] + Y, // HASH COORDINATES OF
        AA = p[A] + Z, // THE 8 CUBE CORNERS,
        AB = p[A + 1] + Z, // THE 8 CUBE CORNERS,
        B = p[X + 1] + Y, // THE 8 CUBE CORNERS,
        BA = p[B] + Z, BB = p[B + 1] + Z;
        return lerp(w, // AND ADD
        lerp(// AND ADD
        v, // AND ADD
        lerp(// AND ADD
        u, // BLENDED
        grad(p[AA], x, y, z), // RESULTS
        grad(p[BA], x - 1, y, z)), // RESULTS
        lerp(// RESULTS
        u, // FROM  8
        grad(p[AB], x, y - 1, z), grad(p[BB], x - 1, y - 1, z))), // CORNERS
        lerp(// CORNERS
        v, // CORNERS
        lerp(// CORNERS
        u, // OF CUBE
        grad(p[AA + 1], x, y, z - 1), grad(p[BA + 1], x - 1, y, z - 1)), lerp(u, grad(p[AB + 1], x, y - 1, z - 1), grad(p[BB + 1], x - 1, y - 1, z - 1))));
    }
