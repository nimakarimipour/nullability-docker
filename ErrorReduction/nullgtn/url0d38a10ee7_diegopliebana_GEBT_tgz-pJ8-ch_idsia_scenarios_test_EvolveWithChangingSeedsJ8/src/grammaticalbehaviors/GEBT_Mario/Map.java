...
public class Map {
    ...
    public static final int MAP_SOFT_OBSTACLE = -11;

    //Strange, but this is another one too.
    public static final int MAP_SOFT_OBSTACLE2 = -76;
    ...
    //public static final int MAP_POT_OR_CANNON = -85;
    //CHANGED FOR 0.1.5
    public static final int MAP_COIN_ANIM = 34;
    ...
    public static boolean isObstacle(byte a_data) {
        return //nothing
        //border hill, jumpable from the bottom.
        a_data != MAP_NOTHING && //Breakable brick
        a_data != MAP_SOFT_OBSTACLE && //Unbreakable brick
        a_data != MAP_SIMPLE_BRICK && //a_data != MAP_POT_OR_CANNON && //Por or cannon for 1 level zoom view
        a_data != MAP_QUESTION_BRICK && //a_data != MAP_BORDER_CANNOT_PASS_THROUGH && //Not sure what is this, but obstacle
        //Muzzle of the cannon
        a_data != //Trunk of the cannon
        MAP_CANNON_MUZZLE && //Flower pot
        a_data != MAP_CANNON_TRUNK && //COIN!
        a_data != MAP_FLOWER_POT && //COIN!
        a_data != MAP_SOFT_OBSTACLE2 && a_data != MAP_COIN_ANIM;
    }
    ...
    public static boolean isPotOrCannon(byte a_data) {
        ...
        //Muzzle of the cannon
        //Trunk of the cannon
        a_data == MAP_CANNON_MUZZLE || //Flower pot
        a_data == MAP_CANNON_TRUNK || a_data == MAP_FLOWER_POT;
    }

    //Defined by us.
    //One space over a solid block
    public static final int MAP_ONE_SPACE_FREE_FLAG = 101;

    //Two spaces over a solid block
    public static final int MAP_TWO_SPACES_FREE_FLAG = 102;
    ...
    public static boolean isSomethingSolid(int a_data) {
        return (//a_data == Map.MAP_POT_OR_CANNON ||
        a_data == Map.MAP_OBSTACLE || a_data == Map.MAP_SIMPLE_BRICK || isPotOrCannon((byte) a_data) || a_data == Map.MAP_QUESTION_BRICK);
    }
                    ...
                    while (vert < 15 && !stop) {
                        ...
                        if (Map.isSomethingSolid(dataInVertical)) {
                            ...
                            if (CanMarioStand(dataOnLeft) > 0) {
                                ...
                                if (//if(dataOnLeftUnder == Map.MAP_POT_OR_CANNON)
                                isPotOrCannon(dataOnLeftUnder)) {
                            ...
                            if (CanMarioStand(dataOnRight) > 0) {
                                ...
                                if (//if(dataOnRightUnder == Map.MAP_POT_OR_CANNON)
                                isPotOrCannon(dataOnRightUnder)) {
