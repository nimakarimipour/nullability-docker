...
public class FollowCurrentPath extends BTLeafNode {
    ...
    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_startingEdge;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_readyToJump;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull int m_xJumpDistance;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull int m_yJumpDistance;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_stopJumping;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_jumpStarted;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull int m_jumpAttempts;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_redoJump;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull float m_lastSpeed;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull int m_forceMove;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull int m_preSlowdownCount;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_jumpPressSpeed;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_grantLRMove;

    private @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull Hashtable<Long, Integer> m_edgeAttempts;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull int m_currentLinkType;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull int m_jumpDestHolesMode;

    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean m_destOnPot;
    ...
    public void resetNode(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull FollowCurrentPath this) {
        m_startingEdge = true;
        m_readyToJump = false;
        m_stopJumping = false;
        m_jumpStarted = false;
    ...
    @org.checkerframework.dataflow.qual.Impure
    public void step(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull FollowCurrentPath this) throws IncorrectNodeException {
        super.step();
        //Get the agent
        ...
        if (marioIndex >= curPath.m_points.size() - 1) {
            ...
            if (marioIndex + 2 < curPath.m_points.size()) {
                ...
                if (nextLink != null)
                    futureLinkType1 = nextLink.getMode();
            }
            ...
            if (marioIndex + 3 < curPath.m_points.size() && nextNodeIdDest != -1) /*this last one shouldnt ever happen*/
            {
                ...
                if (futureLink != null)
                    futureLinkType2 = futureLink.getMode();
            }
            ...
            if (link == null) {
                ...
                switch(link.getMode()) {
                    ...
                    case Graph.MODE_WALK_BIG:
                        ...
                        futureLinkType2 == Graph.MODE_JUMP_BIG || futureLinkType2 == Graph.MODE_JUMP_SMALL || futureLinkType2 >= Graph.MODE_FAITH_JUMP || futureLinkType2 == Graph.MODE_BREAKABLE) {
                            Node dest = mario.getMap().getGraph().getNode(futureNodeIdDest);
                            if (dest != null) {
                                if (futureLink.getMode() >= Graph.MODE_FAITH_JUMP && futureLink.getMetadata() != Edge.META_HOLE)
                                    checkJump = false;
                                if ((futureLink.getMode() == Graph.MODE_JUMP_BIG || futureLink.getMode() == Graph.MODE_JUMP_SMALL) && futureLink.getMetadata() == Edge.META_SOLID)
                                    checkJump = false;
                            }
                            if (checkJump) {
                                ...
                                aboutToJump++;
                                //right too fast
                                shouldISlowDown2 = (marioNode.getX() < destNode.getX()) && (marioSpeedX > 3);
                                //left too fast
                                shouldISlowDown2 |= (marioNode.getX() > destNode.getX()) && (marioSpeedX < -3);
                            }
                        }
                        if (/*futureLinkType1 == Graph.MODE_UNDEF || */
                        futureLinkType1 == Graph.MODE_JUMP_BIG || futureLinkType1 == Graph.MODE_JUMP_SMALL || futureLinkType1 >= Graph.MODE_FAITH_JUMP || futureLinkType1 == Graph.MODE_BREAKABLE) {
                            checkJump = true;
                            Node dest = mario.getMap().getGraph().getNode(nextNodeIdDest);
                            if (dest != null) {
                                if (nextLink.getMode() >= Graph.MODE_FAITH_JUMP && nextLink.getMetadata() != Edge.META_HOLE)
                                    checkJump = false;
                                if ((nextLink.getMode() == Graph.MODE_JUMP_BIG || nextLink.getMode() == Graph.MODE_JUMP_SMALL) && nextLink.getMetadata() == Edge.META_SOLID)
                                    checkJump = false;
                            }
                            if (checkJump) {
                                //I should slow down? Cool Mario, relax!
                                aboutToJump++;
                                //right too fast
                                shouldISlowDown = (marioNode.getX() < destNode.getX()) && (marioSpeedX > 3);
                                //left too fast
                                shouldISlowDown |= (marioNode.getX() > destNode.getX()) && (marioSpeedX < -3);
                                if (shouldISlowDown)
                                    m_preSlowdownCount++;
                    ...
                    case Graph.MODE_JUMP_BIG:
                        ...
                        if (marioNode.getX() == destNode.getX()) {
                            ...
                            if (alreadyUnder) {
                                boolean obs = mario.getMap().checkObstacle(destNode.getX(), marioNode.getY(), destNode.getX(), destNode.getY());
                                if (!obs) {
                                    if (!verticalJump(mario, marioNode, destNode)) {
                                        m_nodeStatus = BTConstants.NODE_STATUS_FAILURE;
                                        mario.setFollowingPath(false);
                    ...
                    default:
                        ...
                        if (link.getMode() >= Graph.MODE_FAITH_JUMP && link.getMode() < Graph.MODE_FAITH_JUMP * 2) {
                            ...
                            if (mario.isMarioLarge()) {
                                ...
                                faithJumpOk = true;
                        } else if (link.getMode() >= Graph.MODE_FAITH_JUMP * 2)
                            faithJumpOk = true;
                        if (faithJumpOk) {
                            //JUMP!!
                            marioNode = mario.getMap().getGraph().getNode(nodeIdMario);
    ...
    @org.checkerframework.dataflow.qual.Impure
    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean manageJump(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull GEBT_MarioAgent mario, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable Node a_marioNode, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable Node a_destNode, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull Edge a_edge) {
        ...
        if (m_startingEdge) {
            ...
            if (mario.isMarioOnLeftOf(a_marioNode.getX())) {
                ...
                if (a_edge.getMode() >= Graph.MODE_FAITH_JUMP && a_edge.getMetadata() != Edge.META_HOLE && m_jumpDestHolesMode != 0)
                    checkSpeed = false;
                if (//0.25) //0.1
                checkSpeed) /*&& Math.abs(marioSpeedX) < 0.25*/
                {
                    m_readyToJump = true;
                    ...
                    if (m_xJumpDistance > 0) {
                        ...
                        if (Math.abs(speed) > 1.0) {
                            ...
                            if (speed > 0.0)
                                ...
                                mario.setAction(Mario.KEY_RIGHT, true);
                            m_readyToJump = false;
                        } else if (//jumping left but looking right.
                        speed > 0) {
                            //1st, wrong way.
                            mario.setAction(Mario.KEY_RIGHT, true);
                            //2nd force LEFT in next cycle.
                            m_forceMove = -1;
                        }
                    }
                } else {
    ...
    @org.checkerframework.dataflow.qual.Impure
    private void manageLeftRight(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull GEBT_MarioAgent mario, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull Node a_destNode,  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull float a_xSpeed) {
        if (m_xJumpDistance > 0) {
    ...
    @org.checkerframework.dataflow.qual.Impure
    private void manageLeftRightInertia(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull GEBT_MarioAgent mario,  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull float a_xInertia,  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull float a_xSpeed, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable Edge a_edge) {
    ...
    @org.checkerframework.dataflow.qual.Impure
    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean verticalJump(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull GEBT_MarioAgent mario, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable Node a_marioNode, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable Node a_destNode) {
        int marioInertia = mario.getInertia();
        float marioSpeedX = mario.getMarioSpeedX();
        if (m_startingEdge) {
            //1.a PRECALCULATION OF THE JUMP
            ...
            if (mario.isMarioOnLeftOf(a_marioNode.getX())) {
                ...
                if (Math.abs(marioSpeedX) < 0.1) {
                    ...
                    //to face correctly and stay at the same spot: 1st in the wrong direction
                    //(where Mario is facing to) and 2nd in the correct one (where Mario should).
                    if (//jumping right but looking left.
                    speed < 0 && m_xJumpDistance > 0) {
                        //1st, wrong way
                        mario.setAction(Mario.KEY_LEFT, true);
                        //2nd force RIGHT in next cycle.
                        m_forceMove = 1;
    ...
    @org.checkerframework.dataflow.qual.Impure
    private  @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull boolean manageFall(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull GEBT_MarioAgent mario, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable Node a_marioNode, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable Node a_destNode) {
        if (m_startingEdge) {
            //NO OPERATION!
