...
public class ReceiptConsoleOuput implements SaleOutputStrategy {
    ...
    public final void outputReceipt(@org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull ReceiptConsoleOuput this, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.Nullable StoreStrategy store, @org.checkerframework.checker.initialization.qual.Initialized @org.checkerframework.checker.nullness.qual.NonNull SaleStrategy sale) {
        String dashes = "----------------------------------------------------------------------------------------------------------";
        String dbdashes = "==========================================================================================================";
        String spaces = "                                                                                                          ";
        System.out.format("%87.87s%n", dbdashes);
        System.out.format("%12.12s %61.61s %12.12s%n", dbdashes, sale.getGreeting(store), dbdashes);
        System.out.format("%87.87s%n", dbdashes);
        System.out.format("%10.10s %30.30s %5.5s %10.10s %10.10s %6.6s %10.10s%n", "Prod ID", "Description", "Qty", "Unit $", "Ext Amt", "Disc", "Line Tot");
        System.out.format("%10.10s %-30.30s %5.5s %10.10s %10.10s %6.6s %10.10s%n", dashes, dashes, dashes, dashes, dashes, dashes, dashes);
        int maxLineItems = sale.getLineItemsLength();
        for (int index = 0; index < maxLineItems; index++) {
            System.out.format("%10.10s %30.30s %5d %10.2f %10.2f %6.2f %10.2f%n", sale.getLineItemId(index), sale.getLineItemDesc(index), sale.getLineItemQuantity(index), sale.getLineItemUnitPrice(index), sale.getLineItemExtendedAmount(index), sale.getLineItemDiscountAmount(index), sale.getLineItemExtendedAmount(index) - sale.getLineItemDiscountAmount(index));
        }
