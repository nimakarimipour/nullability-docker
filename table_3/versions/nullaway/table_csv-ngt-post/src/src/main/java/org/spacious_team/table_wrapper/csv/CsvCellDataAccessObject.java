...
public class CsvCellDataAccessObject implements CellDataAccessObject<RowAndIndex, CsvTableRow> {
    ...
    public CsvCellDataAccessObject(DateTimeFormatter dateTimeFormatter, ZoneId defaultZone) {
    ...
    public Instant getInstantValue(RowAndIndex cell) {
        ...
        //TODO formatter can parse time and zone
        LocalDateTime //TODO formatter can parse time and zone
        dateTime = (value.length() == 10) ? LocalDate.parse(value, formatter).atTime(12, 0) : LocalDateTime.parse(value, formatter);
        return dateTime.atZone(defaultZone == null ? ZoneId.systemDefault() : defaultZone).toInstant();
    }
}
